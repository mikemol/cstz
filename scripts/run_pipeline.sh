...
python3 scripts/calibrate_weights.py 2>&1 | grep -E "^(iter|#)" || true

echo
echo "== Stage 2: parallel-discriminator alignment =="
python3 scripts/align_parallel.py 2>&1 | grep -E "^(#|  \")" || true

echo
echo "== Stage 3: validating against authorial citations =="
python3 scripts/validate_against_comments.py 2>&1 | head -20

echo
echo "== Stage 4: gap / cofiber-cell analysis =="
python3 scripts/gap_analysis.py 2>&1

echo
echo "== Stage 5: rendering reports =="
python3 scripts/write_reports.py 2>&1

echo
echo "Done.  See reports/{pipeline,triples,gaps,conflicts,validation,cofiber,residues}.md"
