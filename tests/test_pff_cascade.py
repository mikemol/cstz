...
class TestSigmaKeyTagProjection:
    """Cover the ``tag_projection`` parameter on ``sigma_key``
    added by Step 1.5.2.

    The parameter exposes both sides of the arrow-category
    functor ``C → C^→`` as orthogonal projections of the same
    substrate, resolving gap #8 by parameterization rather than
    by picking a side.  Two valid values:

    - ``TAG_PROJECTION_LEGACY_THREE_TAG`` (default) — C-side
      projection.  Addr0 sigma_keys start with ``"addr0"``;
      morphism sigma_keys start with ``"morphism"``.  Two
      distinct Addr0s with the same sort have the same κ-key.
      Byte-compatible with Pass 1.
    - ``TAG_PROJECTION_UNIFIED_MORPHISM`` — C^→-side projection.
      All cell sigma_keys start with ``"morphism"``; Addr0
      cells use the ``endpoints`` property
      ``(self.id, self.id)``.  Two distinct Addr0s with the
      same sort have *different* κ-keys (their endpoints
      differ).
    ...
    def test_default_tag_projection_is_legacy_three_tag(self) -> None:
        """``sigma_key(cell)`` (no tag_projection arg) is
        byte-identical to ``sigma_key(cell,
        ...
        assert sigma_key(a0) == sigma_key(
            a0, tag_projection=TAG_PROJECTION_LEGACY_THREE_TAG,
        )
        assert sigma_key(a1) == sigma_key(
            a1, tag_projection=TAG_PROJECTION_LEGACY_THREE_TAG,
        )

    def test_unified_morphism_addr0_starts_with_morphism_tag(
        self,
    ) -> None:
        """Under unified-morphism projection, an Addr0 sigma_key
        starts with ``"morphism"``, not ``"addr0"``."""
        from cstz.pff import (
            Addr0, Pair, Segment, sigma_key,
        ...
        key = sigma_key(
            a, tag_projection=TAG_PROJECTION_UNIFIED_MORPHISM,
        )
        assert key[0] == "morphism"

    def test_unified_morphism_addr0_uses_id_id_endpoints(self) -> None:
        """Under unified-morphism + κ, an Addr0's sigma_key
        contains its id as both endpoints (the
        identity-on-objects convention).  Two distinct Addr0s
        with the same sort have DIFFERENT κ-keys."""
        from cstz.pff import (
            Addr0, Pair, Segment, sigma_key,
            PERSPECTIVE_KAPPA, TAG_PROJECTION_UNIFIED_MORPHISM,
        )
        mk = lambda _id: Addr0(
            id=_id, sort="X",
            segments=[Segment(
        ...
        # Different ids → different (id, id) endpoints → different keys
        assert key_a != key_b
        # Each key contains its own id
        assert "a" in key_a
        assert "b" in key_b

    def test_legacy_kappa_collapses_two_addr0s_with_same_sort(
        self,
    ) -> None:
        """Under legacy + κ, two distinct Addr0s with the same
        sort have the SAME κ-key (the gap #8 status quo).
        This is the *complement* of the unified-morphism test
        above and demonstrates that the two projections produce
        legitimately different equivalence relations."""
        from cstz.pff import (
            Addr0, Pair, Segment, sigma_key, PERSPECTIVE_KAPPA,
        )
        mk = lambda _id: Addr0(
        ...
        # Default tag_projection is legacy-three-tag
        assert sigma_key(mk("a"), perspective=PERSPECTIVE_KAPPA) == \
            sigma_key(mk("b"), perspective=PERSPECTIVE_KAPPA)

    def test_unified_and_legacy_disagree_on_addr0_only(self) -> None:
        """For non-Addr0 cells, both tag projections produce
        identical sigma_keys.  Only Addr0 cells experience a
        non-trivial reading difference."""
        from cstz.pff import (
            Addr1, Addr2, sigma_key, PERSPECTIVE_KAPPA,
            TAG_PROJECTION_LEGACY_THREE_TAG,
            TAG_PROJECTION_UNIFIED_MORPHISM,
        )
        a1 = Addr1(
        ...
        a2 = Addr2(
            id="c", rank="r0", ctor="coh", src="x", dst="y",
        )
        for cell in (a1, a2):
            for perspective in (PERSPECTIVE_KAPPA,):
                legacy = sigma_key(
    ...
    def test_unified_addr1_skips_sort_segments_discriminators(
        self,
    ) -> None:
        """Under unified-morphism, an Addr1 still skips the
        sort/segments discriminators because Addr1 has no sort
        or segments fields.  The unified projection doesn't
        force discriminators that don't apply."""
        from cstz.pff import (
            Addr1, sigma_key, PERSPECTIVE_SIGMA,
            TAG_PROJECTION_UNIFIED_MORPHISM,
        )
        a1 = Addr1(
            id="g", rank="r0", ctor="glue", src="x", dst="y",
        ...
        key = sigma_key(
            a1,
            perspective=PERSPECTIVE_SIGMA,
            tag_projection=TAG_PROJECTION_UNIFIED_MORPHISM,
        )
        # Same shape as legacy-three-tag for Addr1:
        # ("morphism", ctor, src, dst, premises)
        assert key == ("morphism", "glue", "x", "y", ("p",))

    def test_invalid_tag_projection_raises_value_error(self) -> None:
        """An unrecognized tag_projection value raises ValueError
        with a message naming both valid alternatives."""
        from cstz.pff import Addr1, sigma_key
        a1 = Addr1(
            id="g", rank="r0", ctor="glue", src="x", dst="y",
        )
        try:
            sigma_key(a1, tag_projection="bogus")
        except ValueError as e:
            msg = str(e)
            assert "bogus" in msg
            assert "legacy-three-tag" in msg
            assert "unified-morphism" in msg
        else:
            assert False, "expected ValueError on unknown tag_projection"

    def test_unified_perspective_without_endpoints_skips_endpoints(
        self,
    ) -> None:
        """A custom perspective under unified-morphism that
        omits ``DISCRIM_ENDPOINTS`` produces a sigma_key that
        does NOT include the endpoints tuple — exercises the
        false branch of the endpoints check inside
        ``_sigma_key_unified``."""
        from cstz.pff import (
            Addr1, sigma_key, DISCRIM_CTOR,
            TAG_PROJECTION_UNIFIED_MORPHISM,
        )
        ctor_only = frozenset({DISCRIM_CTOR})
        a1 = Addr1(
            id="g", rank="r0", ctor="glue", src="x", dst="y",
        )
        key = sigma_key(
            a1, perspective=ctor_only,
            tag_projection=TAG_PROJECTION_UNIFIED_MORPHISM,
        )
        # Only ("morphism", "glue") — no endpoints, no premises
        assert key == ("morphism", "glue")


class TestArrowCategoryProjection:
    """Cover ``Document.arrow_category_view()`` and the
    ``tag_projection`` axis on ``Document.wedge`` /
    ``Document.wedge_2`` / ``Document.hom_set``.

    The view is a thin forwarding wrapper that bakes
    ``tag_projection=TAG_PROJECTION_UNIFIED_MORPHISM`` into the
    three query methods' defaults.  It is *not* a snapshot —
    mutations to the underlying Document are visible through
    the view (per the three-identity-layers discipline).
    """

    def _doc_with_two_addr0s_same_sort(self):
        """Build a Document with two distinct Addr0s of the
        same sort but different ids.  This fixture exposes the
        legacy-vs-unified κ-key disagreement most cleanly."""
        from cstz.pff import (
            Document, Rank, Patch, Addr0, Pair, Segment,
        )
        return Document(
            documentId="gap8-fixture",
    ...
    def test_arrow_category_view_constructs(self) -> None:
        """``Document.arrow_category_view()`` returns a view
        wrapping the same document."""
        d = self._doc_with_two_addr0s_same_sort()
        view = d.arrow_category_view()
        assert view.document is d

    def test_arrow_category_view_repr_includes_document_id(
        self,
    ) -> None:
        """``__repr__`` shows the wrapped document's id."""
        d = self._doc_with_two_addr0s_same_sort()
        view = d.arrow_category_view()
        s = repr(view)
        assert "gap8-fixture" in s

    def test_legacy_hom_set_collapses_two_addr0s(self) -> None:
        """Under legacy hom_set + κ, the gap #8 collapse holds:
        two distinct Addr0s with the same sort sit in the same
        κ-class, but ``hom_set("a", "a")`` only returns the
        identity at "a" (because the endpoints filter is exact)."""
        d = self._doc_with_two_addr0s_same_sort()
        # ``hom_set("a", "a")`` returns just the identity at "a";
        # the endpoints filter excludes "b" because "b"'s
        # identity has endpoints ("b", "b") not ("a", "a").
        hom = d.hom_set("a", "a")
        assert hom == frozenset({"a"})

    def test_unified_hom_set_uses_unified_kappa_key(self) -> None:
        """Under arrow-category-view hom_set + κ, the
        ``hom_set("a", "a")`` query returns the identity at
        "a" with a unified-morphism κ-key.  The result frozenset
        is the same singleton, but the underlying equivalence
        class structure differs."""
        d = self._doc_with_two_addr0s_same_sort()
        view = d.arrow_category_view()
        hom = view.hom_set("a", "a")
        # Same singleton result as legacy (same identity
        # endpoints filter); the difference is in how the
        # κ-key tuple is shaped, not in the result for this
        # exact query.
        assert hom == frozenset({"a"})

    def test_unified_wedge_2_distinguishes_two_addr0s(self) -> None:
        """The load-bearing gap #8 resolution: under unified
        wedge_2 + (κ, σ), the two Addr0s with the same sort
        sit in DIFFERENT (κ-key, σ-key) buckets because
        their unified κ-keys differ.  Under legacy wedge_2 +
        (κ, σ), they sit in the same κ-bucket because the
        legacy κ-key is ``("addr0", sort)``."""
        from cstz.pff import (
            PERSPECTIVE_KAPPA, PERSPECTIVE_SIGMA,
        )
        d = self._doc_with_two_addr0s_same_sort()
        view = d.arrow_category_view()
        # Under unified, the two Addr0s have different κ-keys
        # → wedge_2 has at least two distinct κ-coordinates
        # for the morphism cells.
        unified_wedge = view.wedge_2(
            PERSPECTIVE_KAPPA, PERSPECTIVE_SIGMA,
        )
        unified_kappa_coords = {key[0] for key in unified_wedge}
        # Under legacy, the two Addr0s have the SAME κ-key
        # → wedge_2 has only one κ-coordinate for them.
        legacy_wedge = d.wedge_2(
            PERSPECTIVE_KAPPA, PERSPECTIVE_SIGMA,
        )
        legacy_kappa_coords = {key[0] for key in legacy_wedge}
        # The unified projection has strictly more distinct
        # κ-coordinates than the legacy projection on this
        # fixture (because it splits the two Addr0s).
        assert len(unified_kappa_coords) > len(legacy_kappa_coords)

    def test_unified_wedge_default_perspectives(self) -> None:
        """``view.wedge()`` (no perspectives) defaults to the
        canonical (σ, τ, κ) triple under unified-morphism.
        Verifies the no-args path through the wedge wrapper."""
        d = self._doc_with_two_addr0s_same_sort()
        view = d.arrow_category_view()
        wedge = view.wedge()
        # Each wedge key is a 3-tuple under the default triple.
        for key in wedge:
            assert len(key) == 3
        # Every cell appears somewhere in the result.
        all_ids = set()
        for ids in wedge.values():
            all_ids.update(ids)
        assert all_ids == {c.id for c in d.cells()}

    def test_unified_hom_set_under_sigma_perspective(self) -> None:
        """``view.hom_set(X, X, perspective=PERSPECTIVE_SIGMA)``
        for an Addr0 X returns the singleton identity, exercising
        the explicit-perspective path through the view's hom_set
        wrapper."""
        from cstz.pff import PERSPECTIVE_SIGMA
        d = self._doc_with_two_addr0s_same_sort()
        view = d.arrow_category_view()
        hom = view.hom_set("a", "a", perspective=PERSPECTIVE_SIGMA)
        assert hom == frozenset({"a"})
