...
\begin{description}[nosep,leftmargin=0.8cm]
  ...
  \item[G8] \emph{Overdetermination residue and
    orthogonalization.} Also discovered by the
    feedback-loop trace: $\gamma$'s $\kappa$-evolution
    (at iter 5) introduced
    \verb|l_gamma_per_pair_per_M| correlated with
    the existing $\oplus$-reduce compare term. The
    correlated parameters over-close the residue at
    some instances and under-close at others,
    producing the four-cell $(1,1)$ signature.
    [DAF]'s prescription for this profile is
    orthogonalization or factoring of the
    correlated axes; the script does not yet
    implement it. \textbf{Open (overdetermination
    signature observed; orthogonalization step
    unimplemented).}$^{d=1}$
\end{description}
