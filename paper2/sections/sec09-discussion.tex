...
$\alpha$ residue plateau (App F) & \cmark &
  \verb|per_pair| form & G7 closed: script added $l_{\alpha,\mathrm{per\_pair}}\approx 29$ \\
$\gamma$ overdetermination (App F) & \cmark &
  rank-deficient blade ($\sigma_{\min}/\sigma_{\max} \approx 10^{-3}$) & G8 closed: script ran SVD, dropped redundant extra \\
$\gamma$ library exhaustion (App F) & \gmark &
  no candidate form fits post-drop residue & G9: enrich candidate-form library \\
...
\noindent G1--G6 were identified by authorial analysis
during paper writing. G7--G9 were \emph{discovered
empirically} by running the feedback-loop script
(Appendix \ref{app:feedback-loop}): G7 when a residue's
monotone-but-decelerating trajectory signalled a
parameter-space shortfall; G8 when the subsequent
$\kappa$-evolution produced a blade whose SVD revealed
rank deficiency; G9 when the post-orthogonalization
residue exhausted the candidate-form library. G7 was
closed in the same run (script added
\verb|l_alpha_per_pair|); G8 was closed in the next
iteration (script dropped the redundant extra after
blade analysis); G9 is the current frontier.

The progression G7 $\to$ G8 $\to$ G9 is itself
[DAF]'s residue-dynamic operating at successively
higher meta-levels: G7 lives in the per-plan parameter
space; G8 lives in the grade-$k$ exterior algebra
over that space; G9 lives in the library of candidate
discriminator-shapes (the meta-space from which new
parameters are drawn). Each higher level's residue
is a discriminator on the level below. The framework
finds its own gaps, and each gap's resolution either
closes the immediate residue or names the next
meta-level residue to articulate.

\subsection{Cofibrative structure between [DAF] and this paper}

The relationship between [DAF] and this paper is not
``predecessor--successor'' in the sense of two independent
works; it is a specific instance of [DAF]'s own fibered
structure. [DAF]'s Theorem 9.6 (Fibered topos) constructs
$\int \mathcal{C}$ over the poset of $\kappa$-states
$\mathbf{K}$ with sub-regime inclusions as morphisms, and
Corollary 9.7 identifies this with a pro-system of topoi
connected by geometric morphisms. This paper sits as a
specific object in that pro-system: the stratum over
the plan-discrimination regime $\Delta$.

The sub-regime inclusion $\Delta \hookrightarrow \Delta'$
(where $\Delta'$ could be [DAF]'s full $\kappa$ or any
larger discrimination context) is the morphism in [DAF]'s
$\mathbf{K}$ that relates this paper's stratum to the
predecessor's. Theorems about $\kappa$ in [DAF]
transport along this inclusion by functoriality;
Prop \ref{prop:op-equals-pareto} is the concrete
realization of that transport. The reflexive governance
--- applying [DAF]'s $\chi$-signature to the paper's
own verification map --- is not decorative; it is
[DAF]'s self-hosting theorem (Theorem 9.14) at work,
with this paper as the internal construction that
reconstructs [DAF]'s framework using [DAF]'s own
substrate.

In [DAF]'s site-of-regimes language (Definition 9.17):
this paper contributes one new object to the site
$\mathfrak{S}$ --- the regime $\Delta$ --- and one
morphism from that object to each previously-existing
object of $\mathbf{K}$ via the appropriate sub-regime
inclusion. The claim of Prop \ref{prop:op-equals-pareto}
is that $\Delta$'s Pareto-frontier structure is covered
(in the Grothendieck-topology sense) by the image of
[DAF]'s adjunction data through the inclusion. Future
papers in the series will extend $\mathfrak{S}$ further,
each contributing a regime and a cover; the pro-system
grows monotonically, consistent with [DAF]'s
$\kappa$-evolution semantics.

This is why the reflexive governance is not ``building
structure to defend our own rigor'': the structure
being applied pre-exists in [DAF] and applies to
every object in the pro-system. An author who notices
the application is not inventing it; they are reporting
a functorial fact about where their paper sits.
