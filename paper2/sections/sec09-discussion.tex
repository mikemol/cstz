...
$\alpha$ residue plateau (App F) & \cmark &
  \verb|per_pair| form & G7 closed: script added $l_{\alpha,\mathrm{per\_pair}}=38.1$ \\
$\gamma$ overdetermination (App F) & \gmark &
  correlated with compare term & G8: orthogonalize, or factor basis (open) \\
...
during paper writing. G7 is qualitatively different:
it was \emph{discovered empirically} by running the
feedback-loop script of Appendix \ref{app:feedback-loop}
and observing a residue whose monotone-but-decelerating
trajectory signalled a parameter-space shortfall. This
is [DAF]'s residue-dynamic at work on the paper's own
meta-claims: a residue that resists articulation under
the current regime demands a $\kappa$-evolution, and
the loop made the demand specific (add
$l_{\mathrm{interp}}$). G7's existence is evidence that
the framework has operational teeth beyond the
authorial-enumeration mode of G1--G6 --- the framework
can find its own gaps, not just confirm the gaps its
author already named.
