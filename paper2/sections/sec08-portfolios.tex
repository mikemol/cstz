...
and the \emph{masks} they apply. We formulate this
constraint in [DAF]'s $\kappa$-truncation language:
atom-sequence identity is equivalence under a
specific sub-regime of the full plan-discrimination
regime $\Delta$.

\begin{definition}[Atom-identity sub-regime]
  \label{def:atom-id-regime}
  Let $\Delta_\mathrm{atom}$ be the truncation
  ([DAF] Remark after Theorem 4.33) of the plan
  discrimination regime that retains only
  \emph{atom-identity discriminators}: for each
  position $i$ in a plan's linearized atom sequence
  and each atom $a \in \mathcal{A}$, the predicate
  ``position $i$ of this plan's atom sequence is
  $a$'' is a $\GF(2)$-valued discriminator on plans.
  $\Delta_\mathrm{atom}$ is the sub-regime consisting
  precisely of these predicates (no encoding, tile,
  mask, or cost discriminators are retained).
\end{definition}

\begin{definition}[$\Delta_\mathrm{atom}$-equivalence]
  \label{def:atom-equiv}
  Two plans $P_1$ and $P_2$ are
  \emph{atom-sequence equivalent}, written $P_1
  \equiv_\mathrm{atom} P_2$, iff they are
  equivalent under $\Delta_\mathrm{atom}$: every
  predicate in $\Delta_\mathrm{atom}$ agrees on
  them.
\end{definition}

Definition \ref{def:atom-equiv} is a direct instance
of [DAF]'s equivalence-under-truncated-regime
construction: by projecting away encoding, tile,
mask, and cost discriminators, we obtain a
quotient-space of plans where two plans are
identified iff their atom sequences coincide
position-by-position. The underlying algebraic
guarantees of [DAF]'s regime machinery transfer
intact: the quotient is a set, the equivalence is
transitive and symmetric by the operationalist
axiom applied to $\Delta_\mathrm{atom}$, and the
truncation residue $\Delta \setminus
\Delta_\mathrm{atom}$ is exactly the information
SIMT dispatch ignores (data, masks) or that
further refinement (encoding, cost) would restore.

\begin{proposition}[Atom-sequence equivalence preserves soundness]
  \label{prop:atom-equiv-sound}
  \footnote{Class \textsf{A}. Warrant: follows
  from soundness of $\kappa$-truncation in [DAF]
  ($\kappa$-equivalence in a truncated regime
  is a coarser equivalence than in the full regime,
  and Grammar Soundness (Thm \ref{thm:soundness})
  is preserved under coarsening). Finite-$\kappa$
  valid.}
  If $P_1 \equiv_\mathrm{atom} P_2$ and both plans
  are sound under Theorem \ref{thm:soundness}, then
  their kernel-level instruction streams coincide
  on every warp lane.
\end{proposition}

\begin{proof}
  Soundness means each plan's dispatched
  instruction sequence corresponds exactly to its
  linearized atom sequence (by induction on the
  grammar). $\Delta_\mathrm{atom}$-equivalence
  means those linearized sequences are pointwise
  equal. Therefore the dispatched instruction
  streams coincide.
\end{proof}
