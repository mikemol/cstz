...
\begin{definition}[Pareto frontier of a workload]
  \label{def:pareto}
  Given a workload $W$, grammar $G$, and budget $B$,
  the \emph{Pareto frontier} $\mathrm{PF}(W, G, B)$ is
  the set of admissible plans $P$ whose cost vector
  $\bar{c}(P)$ is not componentwise dominated by any
  other admissible plan's cost vector.
\end{definition}

\begin{proposition}[Pareto extraction from the chart]
  \label{prop:pareto}
  \footnote{Class \textsf{AH}. Warrant: dominance
  pruning maintains the Pareto frontier as an
  invariant of the chart. Qualifier: assumes the
  chart parser of Section \ref{sec:parsing} prunes
  dominated items eagerly. Finite-$\kappa$ valid.}
  The Pareto frontier $\mathrm{PF}(W, G, B)$ is the
  set of items in the chart cell
  $\chi[0, N]$ whose production is the workload's
  output nonterminal and whose cost vector is
  non-dominated among them.
\end{proposition}

\begin{proof}
  The cost-bounded completer of
  Section \ref{sec:parsing} rejects items with costs
  exceeding $B$, so every surviving item is
  admissible. Pareto-dominance pruning (applied
  eagerly in the chart's insertion step) discards
  items whose cost vector is dominated by another
  item's, so the surviving items at $\chi[0, N]$ for
  the output production are exactly
  $\mathrm{PF}(W, G, B)$.
\end{proof}
